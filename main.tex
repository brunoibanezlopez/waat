...
processes are encoded as step-scoped YAML definitions consumed
incrementally by a super-agent via two dedicated tool calls.  Each
state transition is committed alongside the agent's explicit
reasoning, forming a complete audit record.  The pattern yields
systems that are structurally verifiable before deployment,
resilient to context window degradation, and maintainable by
non-engineers.  We situate WaaT within the mature tradition of
...
benchmark, achieving 93.33\% terminal-state accuracy against 80.00\%
for a workflow-as-prompt baseline, and a 58.71\% reduction in
per-step token usage relative to full-workflow prompting.
\end{abstract}
...
encoded as structured, step-scoped YAML; (ii) incremental,
context-bounded consumption of workflow state via tool calls; and
(iii) mandatory agent reasoning at every state transition, which
...
\textbf{Step-scoped context.}
The agent is never given the full workflow definition.  At each
step it receives only the specification of the current state.
This bounds token consumption, prevents context degradation over
long workflows~\cite{liu2023lost}, and focuses the agent on the
immediate decision.  A fifty-state workflow presents no greater
cognitive load at any individual step than a three-state workflow.
...
process logic and return structured results to the super-agent.
...
scoped to a single step at a time.
...
\texttt{check\_workflow(state\_id)} returns the specification of
the current state only: the action to execute, its parameters, and
available transitions.  The agent has no visibility of past or
future states.
...
The pattern is not appropriate for open-ended tasks, nor for
single-action automations requiring no orchestration.
...
WaaT addresses this problem by exposing workflow state as a
tool-mediated interface rather than as static prompt text.  Instead
of showing the model the entire workflow graph, the orchestration
layer exposes two workflow-control tools.  \texttt{check\_workflow}
returns only the current state's action, action parameters, and
valid outgoing transitions.  \texttt{update\_workflow} advances the
workflow only when the requested transition is valid and the model
supplies non-trivial reasoning.  This design makes the workflow
executable, inspectable, and enforceable at runtime.
...
to a Strands super-agent.  The implementation comprises five
modules: a YAML workflow loader and state-graph validator
(\texttt{workflow.py}), the two core Strands tool definitions
(\texttt{tools.py}), the super-agent loop (\texttt{agent.py}),
a set of mock utility agents (\texttt{mock\_agents.py}), and a
reasoning-quality evaluator (\texttt{evaluator.py}).

For reproducibility, the offline evaluation invokes the
Strands-decorated tools directly while using deterministic mock
utility agents.  This preserves the production integration pattern
while avoiding variance from live model calls in the reported
benchmark.  The \texttt{update\_workflow} tool enforces a minimum
reasoning length at the tool level: calls with fewer than twenty
words in the reasoning field are rejected and the agent must retry
...
\paragraph{Baseline.}
We compare WaaT against a pure workflow-as-prompt baseline in
which the full workflow YAML is placed directly in the system
prompt and the controller predicts the workflow path without
calling \texttt{check\_workflow} or \texttt{update\_workflow}.
This represents a common alternative design: the model receives
the complete procedure as prompt context and is instructed to
follow it.  The baseline has access to the entire workflow
definition but receives no stepwise tool constraints, transition
validation, or mandatory transition reasoning.  Token counts for
both conditions are recorded using the Anthropic API's usage
fields.
...
\begin{enumerate}
  ...
  \item \textbf{Token usage per step}: mean prompt-plus-completion
        tokens per \texttt{check\_workflow}--\texttt{update\_workflow}
        cycle for both WaaT and baseline conditions.
\end{enumerate}
...
\begin{table}[t]
\centering
\caption{Aggregate WaaT results compared with a pure
...
WaaT terminal state accuracy         & 93.33\% \\
Prompt baseline terminal accuracy    & 80.00\% \\
Mean reasoning quality               & 4.86 / 5 \\
Mean WaaT tokens / step              & 142.88 \\
Mean full-workflow tokens / step     & 346.02 \\
Mean prompt baseline tokens / case   & 284.47 \\
Token reduction                      & 58.71\% \\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:waat-results} reports performance under adversarial
stress-test conditions; clean-path evaluation produces 100\%
terminal accuracy across all categories.  Under the adversarial
setting, WaaT achieved 85.00\%
transition accuracy and 83.33\% exact path accuracy under the
adversarial synthetic setting.  The pure workflow-as-prompt
baseline achieved 80.00\% transition accuracy and 73.33\% path
accuracy.  WaaT therefore improved transition accuracy by 5.00
percentage points and exact path accuracy by 10.00 percentage
points.  The largest gap appears in terminal-state accuracy: WaaT
reached the correct terminal state in 93.33\% of cases, compared
with 80.00\% for the prompt baseline.  This suggests that exposing
workflow state through tools improves procedural reliability,
especially when intermediate utility-agent signals are ambiguous
or locally incomplete.
...
followed the intended procedural route.  Some WaaT errors affected
intermediate routing while still reaching the correct terminal
outcome.  By contrast, the prompt baseline was more likely to skip
or misapply procedural states because it made path-level decisions
from the full workflow prompt without stepwise transition checks.

Failures were concentrated in five cases.  Two account-query cases
were routed directly to complaint handling because the requests
contained strong dissatisfaction cues; both still reached the
correct terminal state.  One service-change case should have been
escalated for manual approval but was incorrectly completed by the
mock service agent.  Two edge cases were over-classified rather
than escalated at classification time; one subsequently escalated
via a downstream state, while the other incorrectly completed.
These errors expose realistic production failure modes: ambiguity
at the classification boundary, over-confident utility-agent
outputs, and paths that are locally plausible but globally
suboptimal.

Reasoning quality was high, with a mean score of 4.86 out of 5.
The scoring distribution contained 52 reasoning steps scored at 5
and 4 steps scored at 3.  Lower-scored steps corresponded to
ambiguous classifier outputs, where the generated justification
was adequate but less grounded in specific workflow evidence.
This result supports the claim that the mandatory
\texttt{reasoning} field provides useful audit information while
also surfacing cases where justifications warrant closer review.

WaaT reduced step-level context usage substantially relative to
full-workflow prompting.  The WaaT agent consumed an estimated
142.88 tokens per step compared with 346.02 tokens per step when
the full workflow YAML was included in each step-level prompt,
a 58.71\% reduction.  The pure prompt baseline used an estimated
284.47 tokens per case for one-shot path prediction; although
cheaper in absolute terms for short workflows, it produced lower
path and terminal accuracy because it lacked stepwise transition
validation.  In production settings with many larger workflows,
the context advantage of WaaT is expected to grow because the
agent receives only the current state specification rather than
every possible state across all workflows.
...
apply policy or authorisation gates without a runtime enforcement
mechanism.

WaaT shifts this burden from prompt text into executable tooling.
Workflow state becomes an operational object that the agent
queries and updates, rather than background prose that the agent
is expected to follow.  This makes procedural behaviour more
modular, auditable, and scalable: new workflows can be added as
YAML definitions rather than prompt rewrites; transition
validation is enforced by code; and each update is logged with
explicit reasoning.  Critically, when failures do occur they are
concentrated at known classification boundaries, logged with the
reasoning that produced them, and therefore recoverable---a
stronger operational guarantee than a higher-accuracy system
that fails silently or unpredictably.  These properties are
particularly important for enterprise agent systems, where
production-readiness requires moving beyond prompt-only control
toward architectures that combine language-model flexibility with
software-enforced process boundaries.
...
orchestration paradigms.  By encoding business logic as step-scoped
YAML definitions, exposing workflow state through two well-defined
tool interfaces, and requiring every state transition to carry an
explicit agent-generated justification, WaaT produces agent systems
that are simultaneously flexible, auditable, and maintainable by
non-engineers.
...
framework on a 30-case adversarial synthetic benchmark confirmed
the pattern's key claims: 93.33\% terminal-state accuracy against
80.00\% for a workflow-as-prompt baseline, high reasoning quality
(mean 4.86/5), and a 58.71\% reduction in per-step token usage
relative to full-workflow prompting.  Observed failures were
interpretable and consistent with known boundary conditions in
classification and utility-agent design.
